...
from dpr.data.qa_validation import calculate_matches, calculate_chunked_matches, calculate_matches_from_meta, QAMatchStats
...
def validate(
    ...
    match_stats: QAMatchStats = calculate_matches(passages, answers, result_ctx_ids, workers_num, match_type)
...
def save_results(
    ...
    match_stats: QAMatchStats,
    out_file: str,
):
    # join passages text with the result ids, their questions and assigning has|no answer labels
    merged_data = []
    per_question_hits = match_stats.questions_doc_hits
    ...
    results_with_metrics = {
        ...
        "metrics": {
            "top_k_accuracy": match_stats.top_k_hits,
            "mrr_at_k": match_stats.mrr_k_hits,
            "ndcg_at_k": match_stats.ndcg_k_hits,
        },
        "results": merged_data,
    }

    with open(out_file, "w", encoding="utf-8") as writer:
        writer.write(json.dumps(results_with_metrics, indent=4, ensure_ascii=False) + "\n")
...
@hydra.main(config_path="conf", config_name="dense_retriever_klue")
def main(cfg: DictConfig):
    ...
    if cfg.rpc_retriever_cfg_file and cfg.rpc_index_id:
        ...
        print("ctx_files_patterns: ", ctx_files_patterns)
        print("id_prefixes: ", id_prefixes)
        print("len(ctx_files_patterns): ", len(ctx_files_patterns))
        print("len(id_prefixes): ", len(id_prefixes))
