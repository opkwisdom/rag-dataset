...
class OurJsonlCtxSrc(RetrieverData):
    def __init__(
        self,
        file: str,
        id_prefix: str = None,
        id_attr: str = "id",
        text_attr: str = "contents",
        normalize: bool = False,
    ):
        super().__init__(file)
        self.id_prefix = id_prefix
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage]):
        ...
        with jsonlines.open(self.file, mode="r") as jsonl_reader:
            for jline in jsonl_reader:
                if self.id_prefix:
                    sample_id = self.id_prefix + str(jline[self.id_attr])
                else:
                    sample_id = jline[self.id_attr]
                passage = jline[self.text_attr] if self.text_attr in jline else []
                if self.normalize:
                    passage = normalize_passage(passage)
                ctxs[sample_id] = BiEncoderPassage(passage, None)
                

class OurJsonQASrc(QASrc):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
    ...
    def load_data(self):
        super().load_data()
        data = []
        with open(self.file, mode="r") as json_file:
            json_data = json.load(json_file)
            for item in json_data:
                question = item[self.question_attr]
                answers = item.get(self.answers_attr, [])
                id = item.get(self.id_attr)
                data.append(QASample(self._process_question(question), id, answers))
        self.data = data