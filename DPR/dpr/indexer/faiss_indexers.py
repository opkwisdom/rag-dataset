...
class RerankDenseFlatIndexer(DenseIndexer):
    def __init__(self, buffer_size: int = 2000):
        super(RerankDenseFlatIndexer, self).__init__(buffer_size=buffer_size)
    ...
    def index_data(self, data: List[Tuple[object, np.array]],
        ...
        if passages:
            # Passage IDs만 추출
            passage_ids = set(p[0] for p in passages)
            
            # data 리스트에서 해당 passage에 해당하는 부분만 추출
            data = [d for d in data if d[0] in passage_ids]
