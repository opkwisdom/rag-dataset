...
class CachedPromptRanker:
    ...
    def extract_keyphrases(self, dataloader, doc_list, original_doc_id_list):
        ...
        for docs, cans in tqdm(dataloader, desc="Evaluating "):
            # Unpack data
            ...
            with torch.no_grad():
                ...
                doc_len = (torch.count_nonzero(en_input_ids, dim=-1) - (self.en_temp_len + 1)).unsqueeze(-1)    # (B,)
                pos_scores = pos / doc_len +  self.config['position_factor'] / torch.pow(doc_len, 3)

                batch_scores = gen_scores * pos_scores
                
                # Find Top-k candidates
                topk_scores, topk_indices = torch.topk(batch_scores, k=10, dim=-1)  # (B, top-k)
...
# array([['대화', '조사', '제공', '네트워크', '사용', '데이터', '고려', '확보', '채팅채널', '인터넷 전화']]
