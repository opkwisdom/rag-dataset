...
QAMatchStats = collections.namedtuple("QAMatchStats", ["top_k_hits", "questions_doc_hits", "mrr_k_hits", "ndcg_k_hits"])
...
def calculate_matches(
    ...
    top_k_hits = np.zeros((n_docs,))
    mrr_k_hits = np.zeros((n_docs,))
    ndcg_k_hits = np.zeros((n_docs,))
    
    for question_hits in scores:
        best_hit = next((i for i, x in enumerate(question_hits) if x), None)
        if best_hit is not None:
            best_hit = int(best_hit)
            top_k_hits[best_hit:] += 1
            # MRR Calculation
            mrr_k_hits[best_hit:] += 1.0 / (best_hit + 1)
        
        question_hits = np.array(question_hits)
        actual_k = len(question_hits)
        # NDCG Calculation
        for k in range(1, n_docs + 1):
            # 실제로 존재하는 값만 고려
            top_k_vals = question_hits[:k]
            discounts = np.log2(np.arange(2, 2 + len(top_k_vals)))  # [log2(2), log2(3), ..., log2(k+1)]
            dcg = np.sum(top_k_vals / discounts)

            # 이상적인 정렬 기준
            ideal_vals = np.sort(question_hits)[::-1][:k]
            ideal_discounts = np.log2(np.arange(2, 2 + len(ideal_vals)))
            idcg = np.sum(ideal_vals / ideal_discounts)

            if idcg > 0:    
                ndcg_k_hits[k - 1] += dcg / idcg
    
    num_questions = len(scores)
    mrr_k_hits /= num_questions
    ndcg_k_hits /= num_questions
    
    top_k_hits = list(top_k_hits)
    mrr_k_hits = list(mrr_k_hits)
    ndcg_k_hits = list(ndcg_k_hits)
    
    return QAMatchStats(top_k_hits, scores, mrr_k_hits, ndcg_k_hits)
